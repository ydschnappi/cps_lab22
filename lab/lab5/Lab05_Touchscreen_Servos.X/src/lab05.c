...
void Servo_Init(){
	...
	OC8R = 300; // Write the duty cycle for the first PWM pulse (Duty 1.5 ms)
	OC8RS = 300; // Write the duty cycle for the second PWM pulse (Duty 1.5 ms)
...
unsigned int Touch_Read(int axis){
...
void ADC_Init (){
	...
	//initialize PIN
	SETBIT(TRISBbits.RB15); //set TRISB RB15 to input
	CLEARBIT(AD1PCFGH.PCFG15); //set ADC1 pin AN20 as analog
	//Configure AD1CON1
	...
	//initialize PIN
	SETBIT(TRISBbits.RB9); //set TRISB RB9 to input
	CLEARBIT(AD2PCFGH.PCFG9); //set ADC2 pin AN9 as analog
	//Configure AD1CON1
...
unsigned int ADC_Read (int axis){
	
	if(axis == 0){  
	
		// read X axis
		AD1CHS0bits.CHS0SA = 0x00F; //set ADC1 to sample AN20
		...
		AD2CHS0bits.CHS0SA = 0x009; //set ADC1 to sample AN9
